...
// Get new queries via the repository

let getNewQueries = async() => {
    let query;

    // get query
    query = await queryRepository.getNewQuery();
...
let getRepliedQueries = async() => {
    ...
    query = await queryRepository.getRepliedQueries();
...
let updateReply = async(query) => {
    let updateReplyQuery;
    console.log("queryController update reply: ", query);
    updateReplyQuery = await queryRepository.updateReplyRep(query);

    return updateReplyQuery;
};
