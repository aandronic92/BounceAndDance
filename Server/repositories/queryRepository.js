...
const SQL_SELECT_BY_EMAIL = 'SELECT * FROM QUERIES WHERE userEmail = @email for json path';

const SQL_SELECT_NEW = 'SELECT * FROM QUERIES WHERE instructorId IS NULL ORDER BY queryId DESC for json path';

const SQL_SELECT_REPLIED = 'SELECT * FROM QUERIES WHERE instructorId IS NOT NULL AND instructorReply IS NOT NULL ORDER BY queryId DESC for json path';
...
const SQL_UPDATE_REPLY = 'UPDATE QUERIES SET instructorId = @instructorId , instructorReply = @instructorReply WHERE queryId = @queryId; SELECT * from dbo.QUERIES WHERE queryId = @queryId;'
...
let getQueryByEmail = async(queryEmail) => {
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .input('email', sql.NVarChar, queryEmail)
            // execute query
...
let getNewQuery = async() => {
    ...
    try {
        ...
        const result = await pool.request()
            // execute query
            .query(SQL_SELECT_NEW);

        // Send response with JSON result    
        query = result.recordset[0];

    } catch (err) {
        console.log('DB Error - get new queries: ', err.message);
    }
...
let getRepliedQueries = async() => {
    let query;
    try {
        const pool = await dbConnPoolPromise
        const result = await pool.request()
            // execute query
            .query(SQL_SELECT_REPLIED);
...
let updateReplyRep = async(query) => {
    let updatedQuery;
    console.log("queryController update reply: ", query);
    try {

        const pool = await dbConnPoolPromise
        const result = await pool.request()
            .input('instructorId', sql.Int, query.instructorId)
            .input('instructorReply', sql.NVarChar, query.instructorReply)
            .input('queryId', sql.Int, query.queryId)
            .query(SQL_UPDATE_REPLY);
        updatedQuery = result.recordset[0];
    } catch (err) {
        console.log('DB Error - error updating reply query: ', err.message);
    }
};

// delete a package
let deleteQueryById = async(queryId) => {
